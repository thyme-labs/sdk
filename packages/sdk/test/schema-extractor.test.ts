...
import { extractSchemaFromTask } from '../src/schema-extractor'
...
describe('extractSchemaFromTask — regex & reference edge cases', () => {
	...
	test('escaped quotes remain balanced across scanner stages', () => {
		const schema = parse(
			String.raw`schema: z.object({ note: z.string().describe("say \"hi\", please"), id: z.number() })`,
		)
		expect(Object.keys(schema.properties).sort()).toEqual(['id', 'note'])
	})

	test('an unterminated regex is not mistaken for a line comment', () => {
		const schema = parse(
			'schema: z.object({ value: z.string().regex(/unterminated\n), next: z.number() })',
		)
		expect(Object.keys(schema.properties).sort()).toEqual(['next', 'value'])
	})

	test('returns null for an unbalanced object schema', () => {
		expect(parse('schema: z.object({ value: z.string()')).toBeNull()
	})
})

describe('extractSchemaFromTask — remaining literal and collection forms', () => {
	test('invalid JSON defaults are omitted instead of emitted with the wrong type', () => {
		const schema = parse(
			`schema: z.object({ config: z.record(z.string()).default({ unquoted: true }) })`,
		)
	...
	test('identifier defaults are preserved as unmodelled expressions', () => {
		const schema = parse(
			`schema: z.object({ name: z.string().default(DEFAULT_NAME) })`,
		)
		expect(schema.properties.name.default).toBe('DEFAULT_NAME')
	})

	test('z.record supports one argument, key/value arguments, and no arguments', () => {
		const schema = parse(
			`schema: z.object({
	...
	test('returns null and reports unexpected scanner failures', () => {
		const originalError = console.error
		const errors: unknown[][] = []
		console.error = (...args: unknown[]) => errors.push(args)
		try {
			const throwingInput = Object.defineProperty({}, 'length', {
				get() {
					throw new Error('synthetic scanner failure')
				},
			}) as string
			expect(extractSchemaFromTask(throwingInput)).toBeNull()
